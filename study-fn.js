    function study(){
      ...
      if(!canStudy()){
        return `<p class="eyebrow">Справочник</p><h2 class="study-title">Пока закрыт</h2><p class="study-sub">Откроется после аттестации №1. Аргументаторы PDF — на экране входа и после входа на старте.</p>
        <div style="margin-top:12px">${typeof docsCards==="function"?docsCards():""}</div>
      ...
      return `<p class="eyebrow">${m.brand} · справочник</p>
        ...
        ${body}
        <p style="color:var(--muted);font-size:13px;margin-top:10px;line-height:1.45">${needsRetake()?"Просмотр засчитан для допуска к пересдаче.":expertLocked()?"Справочник открыт. Тренировки по желанию.":"Справочник открыт после аттестации."} PDF — «Открыть» / «Скачать».</p>
