    ...
    function eptsBuildEml(draft, pdf){
      const subj="Гарантийное письмо на ЭПТС · "+draft.carLine;
      const filename=pdf.name;
      const body=[
        "Уважаемый Ян Хуа!",
        "",
        "Направляем гарантийное письмо на выписку ЭПТС.",
        "Автомобиль: "+draft.carLine,
        "Исх. № б/н от "+draft.issued+" г.",
        "Оплата до "+draft.payBy+".",
        "",
        "ООО «ЭКСПЕРТ АВТО САМАРА»",
        "Исполнитель: руководитель отдела продаж Леонтьев А. А.",
        "Тел.: +7 927 724 92 77"
      ].join("\r\n");
      const to=draft.to||"";
    ...
    function eptsMailto(draft){
      const subj="Гарантийное письмо на ЭПТС · "+draft.carLine;
      const body="Уважаемый Ян Хуа!\n\nНаправляем гарантийное письмо на выписку ЭПТС.\nАвтомобиль: "+draft.carLine+"\nИсх. № б/н от "+draft.issued+" г.\nОплата до "+draft.payBy+".\n\nPDF во вложении (скачайте с сайта и приложите вручную — браузер не умеет класть файл в Outlook).\n\nООО «ЭКСПЕРТ АВТО САМАРА»\nЛеонтьев А. А.\n+7 927 724 92 77";
    ...
    function epts(){
      ...
      const step3body = eptsStep===3 && d ? `<div class="epts-body">
        <div class="epts-ok"><b>Вложение из браузера в Outlook напрямую нельзя.</b> Браузер не умеет класть PDF в новое письмо. Ниже два рабочих пути: файл .eml (открывается в Outlook уже с вложением) или скачать PDF и приложить самим.</div>
        <div class="card" style="margin-top:12px">
          <h3 style="margin-top:0">Кого ставить адресатами</h3>
          <p>Генеральному директору ООО «ТЕНЕТ РУС» — <b>Яну Хуа</b>. Почта в открытых реквизитах tenet.ru не опубликована. Вставьте адреса, на которые салон уже пишет импортёру по ЭПТС.</p>
          <label class="field" style="max-width:none"><span>Кому (почта, через запятую)</span>
            <input id="eptsTo" placeholder="адреса импортёра через запятую" value="${escape(d.to||"")}" />
          </label>
          <div class="epts-mail">
            <span class="eyebrow" style="margin:10px 0 0">Тема</span>
            <code>Гарантийное письмо на ЭПТС · ${escape(d.carLine)}</code>
            <span class="eyebrow" style="margin:6px 0 0">Вложение</span>
        ...
        <p class="lead">Файл .eml скачается и откроется в Outlook как неотправленное письмо с PDF. Если система спросит программу — выберите Outlook.</p>
      ...
      return banner("Заказ ЭПТС","Гарантийное письмо импортёру","ЭПТС")+`
        ...
        <div class="epts-steps">
          ...
          <section class="epts-step ${eptsStepClass(3)}">
            <div class="epts-head"><span class="epts-n">3</span><div><h3>Отправить запрос</h3><p>Outlook с вложением — через файл письма. Иначе PDF и адреса вручную.</p></div></div>
            ${step3body}
          </section>
        </div>
        <div class="who-line">
    ...
    function eptsBind(){
      ...
      if(toEl) toEl.oninput=()=>{ if(eptsDraft) eptsDraft.to=toEl.value.trim(); };
      const em=document.getElementById("eptsOutlook");
      if(em) em.onclick=()=>{
        if(!eptsDraft||!eptsDraft.pdf) return;
        if(toEl) eptsDraft.to=toEl.value.trim();
